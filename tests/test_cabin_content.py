...
class CabinContentTests(unittest.TestCase):
    ...
    def test_stylesheet_and_entry_module_versions_match_their_assets(self):
        for filename in ("index.html", "tour.html", "tour-loader.js"):
            source = (ROOT / filename).read_text(encoding="utf-8")
            versions = re.findall(r"(tour\.css|tour-loader\.js|tour-reference-ui\.js)\?v=([\w-]+)", source)
            self.assertTrue(versions)
            for asset, version in versions:
                self.assertEqual(version, "20260911-solid3d1" if asset == "tour.css" else "20260911-reference1")
