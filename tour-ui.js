import { buildScene } from './scene3d.js?v=20260906-3';
...
const VIEWS = {
  ...
  cabin: { name:'水見曉逐', photo:'assets/cabin-2main.jpg', detail:'獨棟木屋・兩間雙人套房' },
};
...
export function mountTour(section, opts = {}) {
  section.querySelectorAll('.tour-reference,.tour-tools,.tour-photo-dialog').forEach(element=>element.remove());
  ...
  function openPhoto(key = current) {
    ...
    dialog.querySelector('img').src=v.photo;dialog.querySelector('img').alt=v.name+'實景照片';
    dialog.querySelector('figcaption').textContent=v.name+'・民宿實景';dialog.showModal();
  }
  ...
  function updateView(key) {
    ...
    canvas.setAttribute('aria-label',view.name+'立體模型');
  ...
  const api = buildScene(canvas, {
    ...
    onPick:key=>{if(key==='estate'||!opts.onPick)openPhoto(key);else opts.onPick(key);},
  });
